...
class _TestResult(TestResult):
    ...
    def addSuccess(self, test):
        ...
        self.result.append((0, test, output, 'Test Success'))
        if self.verbosity > 1:
            sys.stderr.write('OK ')
            sys.stderr.write(str(test))
            sys.stderr.write('\n')
    ...
    def addExpectedFailure(self, test, err):
        ...
        output = self.complete_output()
        self.result.append((0, test, output, 'Expected Fail And Result is Fail: \n'+ _exc_str))
        if self.verbosity > 1:
            sys.stderr.write('ExpectedFail ')
            sys.stderr.write(str(test))
            sys.stderr.write('\n')
        else:
            sys.stderr.write('x')
    ...
    def addUnexpectedSuccess(self, test):
        ...
        output = self.complete_output()
        self.result.append((1, test, output, "Expected Fail But Test Success"))
        if type(getattr(test, "driver", "")).__name__ == 'WebDriver':
            driver = getattr(test, "driver")
            try:
                test.images.append(driver.get_screenshot_as_base64())
                ...
                ...
        if self.verbosity > 1:
            sys.stderr.write('UnexpectedSuccess  ')
            sys.stderr.write(str(test))
            sys.stderr.write('\n')
        else:
            sys.stderr.write('u')
